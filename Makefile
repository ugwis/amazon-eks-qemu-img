...
AVAILABLE_PACKER_VARIABLES := $(shell $(PACKER_BINARY) inspect -machine-readable eks-worker-al2-for-qemu.json | grep 'template-variable' | awk -F ',' '{print $$4}')
...
validate: ## Validate packer config
	$(PACKER_BINARY) validate $(PACKER_VAR_FLAGS) eks-worker-al2-for-qemu.json
...
k8s: validate ## Build default K8s version of EKS Optimized AL2 AMI
	...
	$(PACKER_BINARY) build -timestamp-ui -color=false $(PACKER_VAR_FLAGS) eks-worker-al2-for-qemu.json
